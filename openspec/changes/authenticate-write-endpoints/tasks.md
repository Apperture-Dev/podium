...
- [ ] 1.1 Add an optional bearer-token parameter to `postJson()` in
...
- [ ] 1.2 Write a functional test that posts a team registration carrying an extra
...
- [ ] 2.1 In `services/web/src/app/api/teams/route.ts`, send `Authorization: Bearer <token>` on the
...
- [ ] 2.2 In `services/web/src/app/api/projects/route.ts`, send `Authorization: Bearer <token>` on
      ...
      detail page
- [ ] 2.3 Run `npm run lint` and `npx tsc --noEmit` in `services/web` and verify both are clean,
      then merge and let this deploy on its own before starting group 3

## 3. Second merge, API — close the endpoints

- [ ] 3.1 Write the failing functional tests for the "Write endpoints require an authenticated
      ...
      they return `201`)
- [ ] 3.2 Write the failing functional tests for "A team's creator is the authenticated caller":
...
- [ ] 3.3 Write the failing functional tests for "Registering a project requires membership of the
      target team": a member gets `201`, a non-member gets `403`, an unknown team gets `404`
- [ ] 3.4 Remove `creatorUserId` from `RegisterTeamRequest` and take the creator from
      `#[CurrentUser] UserInterface $user` in `RegisterTeamController`, following
      `ListTeamsController`; verify the tests from 3.2 pass
- [ ] 3.5 Add `string $requestingUserId` to
...
- [ ] 3.6 In `RegisterProjectController`, take the caller from `#[CurrentUser]` and add
...
- [ ] 3.7 Delete the two `PUBLIC_ACCESS` entries for `^/api/teams$` and `^/api/projects$` from
      ...
      tests from 3.1 pass
- [ ] 3.8 Update the OpenAPI attributes on both controllers — drop `creatorUserId` from the team
...
- [ ] 3.9 Run the whole suite with `docker compose exec frankenphp vendor/bin/phpunit` and verify
...
- [ ] 4.1 Remove `creatorUserId` and the `decodeJwtClaims` call from
...
- [ ] 4.2 Verify the unauthenticated path end-to-end: with the session cookie cleared, a write
...
- [ ] 5.1 Update the route list in the root `README.md` to state that both write endpoints now
      require a Bearer token and that team registration takes only `name`
- [ ] 5.2 Verify the hole is actually closed from outside the app: against a running stack,
