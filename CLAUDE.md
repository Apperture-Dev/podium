...
- `docs/hackathon-netpol/` — the per-tenant `NetworkPolicy` set (namespace isolation for hackathon participants' deployed apps), validated 11/11 via `test.sh`. `01-tenant-test.yaml` is the tenant policy fixture; `02-protect-monitoring.yaml` locks down the monitoring namespace from tenant egress.
