...
| Decisión | Razón |
|---|---|
| `templateId` se resuelve vía `TemplateResolver` — adaptador real (2026-09-19): `DoctrineTemplateResolver` consulta el catálogo de `Template` de Build directamente (lectura síncrona entre BCs, mismo patrón que leer `Project.teamId` desde AppManager). Lanza un error claro si no hay ningún `Template` para ese lang/framework | Build ya existe en PHP con un catálogo real (`app:build:seed-templates`) — el placeholder `NotImplementedTemplateResolver` ya no aplicaba, se sustituyó en vez de mantenerlo |
