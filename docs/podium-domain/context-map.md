...
### 2026-09-19 (Build → Deploy, split de infraestructura)
- **`Build` implementado en PHP** (discovery/model cerrados en su propia sesión previa a esta entrada): `Template` (catálogo) + `BuildJob` (un intento). Decisión de alcance nueva: se saca del BC quién lanza el `Job` de Kubernetes — Build solo publica `BuildJobRequested` (imagen, comando, env vars); un lanzador Go mínimo sin lógica de dominio, diferido, lo consumiría. Motivo: un solo lenguaje/convención DDD para todo el dominio del hackathon.
- **`Deploy` confirmado con discovery propio** (`deploy/discovery.md`, `deploy/model.md`) — hasta ahora solo tenía responsabilidad e invariantes heredados de la sesión de arquitectura original, sin BC propio abierto. Aggregate root: `DeployAttempt`, paralelo a `BuildJob`. Mismo split que Build: publica `DeployAttemptRequested`; el lanzador Go (mismo componente que el de Build, ampliado, o uno nuevo — decisión de infraestructura diferida) solo hace `kubectl apply` del CR `Application` de ArgoCD.
- **Simplificación importante, aplicable a todo el mecanismo de despliegue**: no hay una imagen OCI de manifiestos por build — el chart de Kubernetes (`podium-app`) es genérico y se publica una sola vez, fuera del ciclo de cada build/deploy (ya estaba en el plan de hackathon original, se había pasado por alto al modelar Build). `DeployAttempt` solo junta los `values` (imagen, env vars, base de datos) que ese chart necesita.
- Resuelto en Deploy: los 3 reintentos de health-check (invariante ya confirmada, fila de arriba) los cuenta la infraestructura (ArgoCD/el lanzador Go), no el dominio — `DeployAttempt` puede guardar el conteo como dato informativo si la señal terminal lo trae, sin contarlo él mismo mensaje a mensaje.
