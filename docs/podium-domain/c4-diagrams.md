...
| **Catálogo de plantillas** | Cinco: `nodejs/nestjs`, `nodejs/nextjs`, `nodejs/react`, `nodejs/vue` y `python/fastapi`. Otro lenguaje es un Dockerfile más en `services/build-runner/templates/` y una línea más en `app:build:seed-templates` |
