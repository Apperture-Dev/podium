...
| **Secretos / env vars del tenant** | El chart todavía no inyecta las env vars declaradas en `environment:`, y la pantalla de secretos del dashboard es fixture de frontend. La **base de datos sí** llega: `database` viaja resuelto hasta el chart, que monta un `Cluster` de CNPG por servicio |
| **Base de datos por tenant (CNPG)** | Implementada: Deploy lee el bloque `database:` del `podium.yaml` y el chart monta el `Cluster`, inyectando las credenciales que genera CNPG en las env vars que pidió el equipo |
| **Catálogo de plantillas** | Diez combinaciones en cinco lenguajes: `nodejs` (nestjs, nextjs, react, vue), `python` (fastapi), `php` (symfony, symfony-worker, laravel), `go` (stdlib), `rust` (cargo). Otra más es un Dockerfile en `services/build-runner/templates/` y una línea en `app:build:seed-templates` |
